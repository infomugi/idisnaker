...
class Subdistrict extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('subdistrict_code, mnemonic, name', 'required'),
			array('subdistrict_code', 'numerical', 'integerOnly'=>true),
			array('mnemonic', 'length', 'max'=>2),
			array('name', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_subdistrict, subdistrict_code, mnemonic, name', 'safe', 'on'=>'search'),
		);
	}

...
class RSA extends CActiveRecord
{
	...
	$end=rand(3000,4000);
	for($e=$start;$e<$end;$e++){  
	    $gcd = gmp_gcd($e, gmp_intval($totient));
	    if(strval($gcd)=='1')
	        break;
	}
		return $e;
	...
	public function generatePrivateKeys($totient,$e){
		$i=1;
		do{
			$a = gmp_mul(gmp_intval($totient),$i);
			$b = gmp_add($a,1);
			$res = gmp_div_qr($b, $e);
			$i++;
		    if($i==10000) 
	...
	public function encrypt($plaintext,$e,$n){
		...
		for($i=0;$i<strlen($plaintext);++$i){
			...
			$ciphertext .= gmp_intval(gmp_powm($todecimal,$e,$n),36);
	...
	public function decrypt($ciphertext,$d,$n){
		...
		foreach($teks as $nilai){
			...
			$plaintext .= chr(gmp_intval(gmp_powm($decimal, $d, $n)));

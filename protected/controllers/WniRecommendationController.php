...
class WniRecommendationController extends Controller
{
	/**
	...
	{
		$data=WniRecommendation::model()->findByPk($id);
		$cek=WniRecommendation::model()->findByPk($id);
	...
	{
		$this->layout = "signin";
		$verifikasi=new WniRecommendation;
		$verifikasi->setScenario('verify');

		$data=$this->loadEncrypt($verify);

		if(isset($_POST['WniRecommendation']))
		{
			$verifikasi->attributes=$_POST['WniRecommendation'];
			...
			if($verifikasi->privatekey==""){
				...
				Yii::app()->user->setFlash('Success','WniRecommendation '.$data->no_registration.' Valid');
	...
	// {
	// 		$this->layout = "signin";
	// 		$model=WniRecommendation::model()->findByPk($id);
	// 		$model->views += 1;
	...
	{
		$this->layout = "signin";
		$model=WniRecommendation::model()->findByAttributes(array('encrypt'=>$verify));
		$model->views += 1;
		$model->last_view = date('Y-m-d h:i:s');
		$model->save();	
	...
	{
		$data=WniRecommendation::model()->findByAttributes(array('encrypt'=>$verify));
		if($data===null)
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['WniRecommendation']))
		{
			$model->attributes=$_POST['WniRecommendation'];
	...
	{
		$dataProvider=new CActiveDataProvider('WniRecommendation');
	...
	{
		$dataProvider=new CActiveDataProvider('WniRecommendation',array('criteria'=>array('condition'=>'citizenship_id='.$id)));
	...
	{
		$model=new WniRecommendation('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['WniRecommendation']))
			$model->attributes=$_GET['WniRecommendation'];
	 ...
	 * @return WniRecommendation the loaded model
	 ...
	 * @param WniRecommendation $model the model to be validated
	 */

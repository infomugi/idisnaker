...
class SiteController extends Controller
{
	...
	public function actionReset($token)
	{
		...
		if(isset($_POST['Users']))
		{
			...
			if($model->save())
				...
				$updatetoken=Users::model()->findByAttributes(
	...
	public function loadUser($id)
	{
		...
		if($model===null)
			throw new CHttpException(404,'Data pengguna tidak ditemukan.');
		return $model;
	}		

	public function loadCertificate($verify)
	{
		$model=Certificate::model()->findByPk($verify);
		if($model===null)
			throw new CHttpException(404,'Kunci yang anda masukan tidak valid.');
		return $model;
	}			
	 ...
	 * Displays the : Forgot Password with active email and send verification to mail
	 */
	public function actionEncrypt($verify)
	{
		$this->layout = "signin";
		Yii::import('ext.yii-mail.YiiMailMessage');
		$email = new YiiMailMessage;
		...
		if(isset($_POST['ForgotPassword']))
		{
			$verify->attributes=$_POST['ForgotPassword'];

			//Find User
			$findcertificate=$this->loadCertificate($forgot->privatekey);

			//Find User
			$finduser=$this->loadUser($findcertificate->member_id);
			$user_firstname = $finduser->first_name;
			$user_username = $finduser->username;
			$user_email = $finduser->email;
